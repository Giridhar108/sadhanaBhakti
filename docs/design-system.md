...
Тени в текущем направлении dashboard не используются.

Правило:

- для карточек использовать `box-shadow: none`;
- не использовать `filter: drop-shadow(...)` и декоративный `text-shadow`;
- отделять компоненты цветом, мягкими градиентами и спокойными подложками;
- не добавлять hover-тени и декоративные drop-shadow без отдельной просьбы;
